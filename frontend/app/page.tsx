...
const FALLBACK_IMAGE = "https://images.unsplash.com/photo-1542291026-7eec264c27ff?w=1600&q=80";

export default async function HomePage() {
  // Never crash — always fall back to empty array
  let allProducts: Awaited<ReturnType<typeof getAllProducts>> = [];
  try {
    allProducts = await getAllProducts();
  } catch {
    // API unavailable — show page without products
  }
    ...
    <div className="min-h-screen bg-white flex flex-col">
      ...
      <section className="relative bg-neutral-900 text-white overflow-hidden">
        <div className="absolute inset-0">
          <img
            src={heroProduct?.variants?.[0]?.images?.[0] ?? FALLBACK_IMAGE}
            alt="Hero"
        ...
        <div className="relative max-w-7xl mx-auto px-6 py-28 sm:py-40">
          ...
          <div className="flex items-center gap-4 flex-wrap">
            <Link
              href={heroProduct ? `/product/${heroProduct.id}` : "/men"}
      ...
      <section className="max-w-7xl mx-auto px-6 py-20">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {categories.map((cat) => (
            <Link key={cat.href} href={cat.href} className="group relative aspect-[3/4] rounded-2xl overflow-hidden bg-neutral-100">
