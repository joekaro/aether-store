...
interface VariantForm {
  color: string; colorHex: string; size: string; price: string; stock: string; images: string;
}
...
export default function NewProductPage() {
  ...
  const [form, setForm] = useState({ name: "", brand: "AETHER", category: "Running", description: "", basePrice: "" });
  const [variants, setVariants] = useState<VariantForm[]>([{ color: "", colorHex: "#000000", size: "", price: "", stock: "", images: "" }]);
    ...
    try {
      ...
      const payload = {
        ...
        variants: variants.map((v, i) => ({
          id: `${slug}-v${i}`,
          color: v.color, colorHex: v.colorHex, size: v.size,
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-6">
          <div className="bg-white rounded-2xl border border-neutral-100 p-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="sm:col-span-2">
                ...
                <input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} required placeholder="e.g. Aether Runner Pro" className="w-full h-11 px-4 text-sm border border-neutral-200 rounded-xl outline-none focus:border-neutral-900 transition-colors" />
              </div>
              <div>
              ...
              <div>
                ...
                <input type="number" value={form.basePrice} onChange={(e) => setForm({ ...form, basePrice: e.target.value })} required placeholder="180" className="w-full h-11 px-4 text-sm border border-neutral-200 rounded-xl outline-none focus:border-neutral-900 transition-colors" />
              </div>
              <div>
              ...
              <div className="sm:col-span-2">
                ...
                <textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} rows={3} required placeholder="Describe this product..." className="w-full px-4 py-3 text-sm border border-neutral-200 rounded-xl outline-none focus:border-neutral-900 transition-colors resize-none" />
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-neutral-100 p-6">
            <div className="flex items-center justify-between mb-5">
              ...
              <button type="button" onClick={() => setVariants((prev) => [...prev, { color: "", colorHex: "#000000", size: "", price: "", stock: "", images: "" }])} className="text-xs font-semibold border border-neutral-200 px-3 py-1.5 rounded-lg hover:bg-neutral-50 transition-colors">+ Add Variant</button>
            </div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                    {[
                      ...
                      <div key={field}>
                        ...
                        <input type={["price","stock"].includes(field) ? "number" : "text"} value={v[field as keyof VariantForm]} onChange={(e) => updateVariant(i, field, e.target.value)} placeholder={placeholder} required className="w-full h-9 px-3 text-sm border border-neutral-200 rounded-lg outline-none focus:border-neutral-900 bg-white transition-colors" />
                      </div>
                    ))}
                    <div>
                    ...
                    <div className="sm:col-span-2">
                      ...
                      <input value={v.images} onChange={(e) => updateVariant(i, "images", e.target.value)} placeholder="https://image1.jpg, https://image2.jpg" className="w-full h-9 px-3 text-sm border border-neutral-200 rounded-lg outline-none focus:border-neutral-900 bg-white transition-colors" />
                    </div>
                  </div>
                </div>
              ))}
